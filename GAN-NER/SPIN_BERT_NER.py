...
flags.DEFINE_string("middle_output", "msr_middle_data", "Dir was used to store middle data!")
...
class DataProcessor(object):
    ...
    def _read_data(cls, input_file):
        ...
        rf = open(input_file, 'r', encoding='utf-8').readlines()
        lines = []
        words = []
        labels = []
        count = 0
        ...
        for i, line in enumerate(rf):
            if len(line.strip().split()) == 2:
                ...
                data = line.strip()
                word_str, labels = data.split('\t')
                word_str = strQ2B(word_str)
                word_str = word_str.replace('“', '"').replace('‘', "'") \
                    .replace('”', '"').replace('’', "'") \
                    .replace('—', "_").replace('…', "_")
                lines.append((labels, word_str))
        return lines
...
class CWSProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "train.char.cnc")), "train"
        )

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "dev.char.cnc")), "dev"
        )

    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "test.char.cnc")), "test"
        )
...
def convert_single_example(ex_index, example, max_seq_length, tokenizer, label_map):
    """
        ...
        for i, _ in enumerate(token):
            if i == 0:
                ...
                if is_number(token[0]):
                    ...
                    symbol.append(0)
                if str(token).encode('UTF-8').isalpha():
                    symbol.append(1)
                else:
                    symbol.append(0)
                if str(token[-1]) in symbollist:
                    symbol.append(1)
                else:
                    symbol.append(0)
    ...
    if len(tokens) >= max_seq_length - 1:
        ...
        symbols = symbols[0:(max_seq_length - 1)]
        content = content[0:(max_seq_length - 1)]
    ntokens = []
    segment_ids = []
    label_ids = []
    ntokens.append("[CLS]")
    ...
    while len(input_ids) < max_seq_length:
        ...
        ntokens.append("[PAD]")
        content.append("[PAD]")
    assert len(symbols) == max_seq_length
    ...
    assert len(content) == max_seq_length
    if ex_index < 3:
        ...
        logging.info("symbols: %s" % " ".join([str(x) for x in symbols]))
    ...
    return symbols, feature, content, label_ids


def filed_based_convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, output_file, label_map,
                                             mode=None):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if not example.label or not example.text:
            continue
        symbols, feature, ntokens, label_ids = convert_single_example(ex_index, example, max_seq_length, tokenizer,
                                                                      label_map)
        ...
        def create_float_list_feature(values):
            f = tf.train.FeatureList(
                feature=[tf.train.Feature(float_list=tf.train.FloatList(value=list(value))) for value in values])
            return f

        features = collections.OrderedDict()
        symbols_list = create_float_list_feature(feature.symbols)
        # symbols_features = create_float_list_feature(feature.symbols)
        ...
        tf_example = tf.train.SequenceExample(
            ...
            feature_lists=tf.train.FeatureLists(feature_list={'symbols': symbols_list})
        )
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    ...
    name_to_feature_lists = {
        "symbols": tf.FixedLenSequenceFeature([SYMBOL_SIZE], tf.float32),
    }

    def _decode_record(record, name2context_features, name2feature_lists):
        context_example, sequence_example = tf.parse_single_sequence_example(record,
                                                                             context_features=name2context_features,
                                                                             sequence_features=name2feature_lists)
    ...
    def input_fn(params):
        ...
        d = d.apply(tf.data.experimental.map_and_batch(
            lambda record: _decode_record(record, name_to_context_features, name_to_feature_lists),
...
def symbols_emb(symbols, num_symbols_size):
    linear = tf.keras.layers.Dense(num_symbols_size, activation=tf.nn.relu)
    return linear(symbols)
...
def create_model(bert_config, is_training, symbols, input_ids, mask,
    ...
    _shape = modeling.get_shape_list(input_ids)

    # output_layer shape is
    if is_training:
        output_layer = tf.keras.layers.Dropout(rate=0.1)(output_layer)

    symbols_size = symbols.shape[-1].value
    batch_size = _shape[0]
    hidden_size = output_layer.shape[-1].value + symbols_size
    symbols = symbols_emb(symbols, symbols_size)
    output_layer = tf.concat([output_layer, symbols], 2)

    pading = tf.zeros(shape=[batch_size, 1, hidden_size])

    # bigram
    head = tf.concat([pading, output_layer[:, :-1, :]], 1)
    tail = output_layer[:, :, :]
    bi_gram = tf.concat([head, tail], 2)

    logits = hidden2tag(bi_gram, num_labels)
    # TODO test shape
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        if FLAGS.crf:
            (total_loss, logits, predicts) = create_model(bert_config, is_training, symbols, input_ids,
...
def main(_):
    ...
    processor = CWSProcessor()

    label_list = processor.get_labels()
    label_map = save_id2label(label_list)
    ...
    if FLAGS.do_train:
        ...
        if not os.path.exists(train_file):
            _, _ = filed_based_convert_examples_to_features(train_examples, label_list, FLAGS.max_seq_length, tokenizer,
                                                            train_file, label_map)
...
if __name__ == "__main__":
    FLAGS.data_dir = '../data/msr'
    FLAGS.output_dir = './msr_output'
